...
import {
  descargarReguladoraPDF,
  fmt,
  fmtPct,
  type CalculoHistorial,
} from '../historial'

export default function NuevaSimulacionVaporPage() {
  const searchParams = useSearchParams()
  const [p1Str, setP1Str] = useState(searchParams.get('p1') ?? '4')
  const [p2Str, setP2Str] = useState(searchParams.get('p2') ?? '2')
  const [qStr, setQStr] = useState(searchParams.get('q') ?? '300')
  ...
  const handleDescargarPDF = async () => {
    ...
    try {
      ...
      toast.success(`Cálculo guardado en el historial (${guardado.medida ?? 'fuera de rango'})`)
    } catch {
        ...
        <div>
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" asChild>
              <Link href="/herramientas/calculadora-vapor">
