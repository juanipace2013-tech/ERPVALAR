...
export async function colppyCreateInvoice(
  ...
  const payload = {
    ...
    parameters: {
      ...
      netoNoGravado: Number(invoice.netoNoGravado) || 0,
      exento: Number(invoice.exento) || 0,
      totalIVA: Math.round(Number(invoice.totalIVA) * 100) / 100,
      IVA21: Math.round(Number(invoice.IVA21) * 100) / 100,
      IVA105: Number(invoice.IVA105) || 0,
      IVA27: Number(invoice.IVA27) || 0,
      noGravado: Number(invoice.noGravado) || 0,
      percepIVA: Number(invoice.percepIVA) || 0,
      percepIIBB: Number(invoice.percepIIBB) || 0,
      impInterno: Number(invoice.impInterno) || 0,
