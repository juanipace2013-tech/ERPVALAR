...
- Respondé SOLO con la información que te dan: la ficha técnica del fabricante, datos de la publicación, del producto en el ERP y ejemplos de respuestas anteriores del mismo ítem. NO inventes medidas, materiales, presiones, certificaciones, compatibilidades, plazos ni precios.
- Todo dato técnico (tipo de rosca y norma, materiales, presión y temperatura de trabajo, dimensiones, conexiones, certificaciones, compatibilidad con fluidos) tiene que salir de la sección "Ficha técnica del fabricante". Si la ficha dice algo distinto que la descripción del ERP o de la publicación, vale la ficha. Si la pregunta es técnica y NO hay ficha técnica (o la ficha no cubre ese dato), marcá needs_review=true e indicá en review_reason qué dato falta confirmar; no lo completes con conocimiento general.
- Citá las normas y especificaciones tal como figuran en la ficha, sin agregar interpretaciones propias (por ejemplo, no digas "rosca paralela" o "cónica" si la ficha solo dice "ISO 7-1"). Si preguntan si sirve para un fluido o aplicación (vapor, gas, agua potable, aceite, etc.) y la ficha no lo menciona explícitamente, no afirmes que sirve: dá los datos de presión/temperatura/materiales que sí figuran y marcá needs_review=true.
...
export interface ErpProductContext {
  ...
  stockQuantity: number
  unit: string
  technicalSheetText?: string | null // texto extraído del PDF de la ficha
  family?: FamilyVariantContext[]
}

// Las fichas suelen tener 1-4 páginas (~3-8k chars); el tope evita que un
// catálogo subido como ficha se coma el prompt.
const TECHNICAL_SHEET_PROMPT_MAX_CHARS = 8000
...
export async function generateAnswer(input: AnswerInput): Promise<AnswerResult> {
  ...
  if (product) {
    ...
    parts.push('')
    parts.push('## Ficha técnica del fabricante')
    if (product.technicalSheetText) {
      parts.push('"""')
      parts.push(product.technicalSheetText.slice(0, TECHNICAL_SHEET_PROMPT_MAX_CHARS))
      parts.push('"""')
    } else {
      parts.push('(este producto no tiene ficha técnica cargada; no afirmar datos técnicos que no estén en la publicación o en el ERP)')
    }
